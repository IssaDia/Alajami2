...
export default function FormWidget() {
  ...
  const optionsData = data.allContentfulBlogCategories.edges.map(
    ({ node }, index) => {
      ...
      <Box title="Formulaire de contact">
        <Formik
          ...
          onSubmit={(values, { setSubmitting }) => {
            setSubmitting(true);
            axios
              .post('http://localhost:5000/contact/send', values)
        ...
        >
          {({
            ...
            <form
              ...
              className="w-full max-w-xl space-y-10 p-16 font-noticia"
            >
              <div className="w-full px-3">
                ...
                {touched.email && errors.email ? (
                  <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    <span className="helper-text text-red-400">
                      {errors.email}
                    </span>
                  </div>
                ) : null}
              ...
              <div className="w-full text-center">
                <Button
                  styles="bg-gray-600 hover:bg-green-500 text-white font-bold py-2 px-4 border-b-4 border-gray-900 hover:border-green-900 rounded"
                  type="submit"
                  name="Soumettre"
                />
              </div>
